...
//                preserveComments: true
    ...
    grunt.registerTask('run-wiremock', [
        'wiremock',
        'express-keepalive'
    ]);

    grunt.registerTask('wiremock', function () {
        var done = this.async(),
            startupDelay = 2000,
            wiremockPort = 9997;

        grunt.log.writeln('Starting wiremock on port ' + wiremockPort);

        var wmi = grunt.util.spawn({
            cmd: 'java',
            args: ['-Xms512m', '-Xmx512m', '-jar', 'wiremock.jar', '--port', wiremockPort],
            opts: {
                cwd: './wiremock'
            }
        }, function (error, result, code) {
            grunt.log.writeln('wiremock ended', error, result, code);
        });

        wmi.stdout.on('data', function (buf) {
            console.log(String(buf));
        });
        wmi.stderr.on('data', function (buf) {
            console.log(String(buf));
        });


        // I can't find any way to hook into Grunt exiting, so have to do it when node exits...
        process.on('exit', function () {
            wmi.kill();
        });

        grunt.log.writeln('Giving wiremock ' + startupDelay + 'ms to start up...');
        setTimeout(function () {
            grunt.log.writeln('Assuming wiremock is started now!');
            done();
        }, startupDelay);
    });
